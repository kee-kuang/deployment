...
systemctl status firewalld | grep loaded
[ $(echo $?) -eq 0 ] && echo "firewall closed successfully!"
...
#如果安装有报错，需要执行禁用所有插件或者 --nogpgcheck 禁止进行gpg check
#yum -y install https://download.postgresql.org/pub/repos/yum/reporpms/EL-7-x86_64/pgdg-redhat-repo-latest.noarch.rpm -nogpgcheck  > /dev/null 
