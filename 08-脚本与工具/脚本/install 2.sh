...
#cp ../*.conf /usr/local/nginx/conf/conf.d/
rm -rf /usr/local/nginx/conf/conf.d/nginx.conf
mkdir -p /usr/local/nginx/html/{admin,platform,h5}
...
create_dir(){
mkdir -p /data/{es,rabbitmq,redis,nacos,logs}
mkdir -p /data/es/conf
chmod -R 775 /data/es
chown -R www. /data/es
chown -R www. /data/logs
cp -r basic /data/
cp -r xxl-job-admin /data/
sleep 2
...
product_init(){
SERVER_IP=$(ip -4 addr show eth0 | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | head -1)
...
cd /data/lingxi
sed -i "s/^export LINGXI_NACOS_ADDR=127.0.0.1/export LINGXI_NACOS_ADDR=$SERVER_IP/" .config
}
